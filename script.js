...
async function getJoke() {
    let joke = '';
    let apiUrl = 'https://v2.jokeapi.dev/joke/Programming?blacklistFlags=nsfw,religious,political,racist,sexist,explicit';
    try {
     const response = await fetch(apiUrl);
     let apiJoke = await response.json();
     if (apiJoke.setup) {
      joke = `${apiJoke.setup} ... ${apiJoke.delivery}`;
     } else {
      joke = apiJoke.joke;
     }
    //  Text-to-Speech
...
// Say the joke using the text to speech API
